...
export const SITUATION_TEMPLATES: Record<MaturityLevel, string> = {
  ...
  performing:
    '{dealerName} demonstrates advanced performance, with structured processes, defined KPIs, and consistent execution across most departments. The assessment identifies selective improvement opportunities that, if addressed, would allow the dealership to reach top-quartile performance in targeted areas. Top-quartile dealers at this level focus their improvement energy on the 20% of initiatives that drive 80% of incremental performance gains.',

  advanced:
    '{dealerName} is performing at an advanced level across the assessed dimensions, with strong process discipline, data-driven decision-making, and consistent above-benchmark results. The assessment has identified ceiling-level opportunities — areas where the dealership can move from best-practice execution to market-differentiating performance. Dealers operating at this level typically focus on innovation, digital capability, and talent development to maintain competitive distance from the market.',
};
...
export const PRIORITY_TEMPLATES: Record<MaturityLevel, string> = {
  ...
  advanced:
    'At advanced maturity, the priority shifts from operational improvement to competitive differentiation — the focus is on the capabilities that will separate this dealership from peers over the next 12–24 months. The assessment has identified ceiling-level opportunities in specific areas where best-practice execution can be elevated to top-quartile performance through technology, talent, or process innovation. Sustaining top-quartile performance requires quarterly reassessment to ensure the dealership continues to raise its own benchmark rather than optimising toward a static target.',
};
