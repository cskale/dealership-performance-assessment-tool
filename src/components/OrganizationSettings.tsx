...
import { Save, Upload, X, Building2, ShoppingCart, Globe, Star, Palette, HelpCircle, Pencil, Loader2, CheckCircle, MapPin, Languages, Briefcase, Network, Info } from 'lucide-react';
...
export const OrganizationSettings = ({ organizationId, isAdmin }: Props) => {
  ...
  if (complete && !editMode) {
    const languageLabel =
      settings.default_language === 'de' ? 'Deutsch' :
      settings.default_language === 'fr' ? 'Français' :
      settings.default_language === 'es' ? 'Español' :
      settings.default_language === 'it' ? 'Italiano' : 'English';

    const Field = ({ label, children, autoDerived }: { label: string; children: React.ReactNode; autoDerived?: boolean }) => (
          ...
          {autoDerived && (
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent><p className="text-xs">Auto-derived from brand portfolio</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
        </div>
        <div className="mt-1.5 text-[15px] font-medium text-foreground">{children}</div>
      </div>
    );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground/80 mt-1 flex items-center gap-1.5">
              <CheckCircle className="h-3 w-3 text-[hsl(var(--dd-green))]" /> Configuration complete
            </div>
          </div>
          {isAdmin && (
            <Button variant="outline" size="sm" onClick={() => setEditMode(true)} className="text-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 divide-y sm:divide-y-0 divide-border">
          <Field label="Brands">
            <div className="flex flex-wrap gap-1.5">
              {(settings.oem_brands || []).map(b => <BrandChip key={b} label={b} />)}
            </div>
            <div className="text-xs text-muted-foreground font-normal mt-1.5">
              {settings.brand_mode === 'single_brand' ? 'Single brand' : 'Multi-brand'} · <span className="capitalize">{settings.oem_authorization}</span>
            </div>
          </Field>

          <Field label="Market position" autoDerived>
            {TIER_LABELS[settings.positioning || ''] || '—'}
          </Field>

          <Field label="Location">
            {settings.city}, {settings.country}
            <div className="text-xs text-muted-foreground font-normal mt-0.5 capitalize">
              {settings.network_structure?.replace('_', ' ')}
            </div>
          </Field>

          <Field label="Business model">
            <span className="capitalize">{settings.business_model?.replace(/_/g, ' ')}</span>
            <div className="flex flex-wrap gap-1 mt-1.5">
              {(settings.product_segments || []).map(s => (
                <span key={s} className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-[hsl(var(--dd-fog))] text-muted-foreground border border-[hsl(var(--dd-rule))] capitalize">
                  {s.replace('_', ' ')}
                </span>
              ))}
            </div>
          ...
          <Field label="Reporting language">
            {languageLabel}
            <div className="text-xs text-muted-foreground font-normal mt-0.5">Used for exports & reports</div>
          </Field>
          ...
          <Field label="Brand identity">
            {settings.logo_url ? (
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground font-normal">Custom logo uploaded</span>
              </div>
            ) : isAdmin ? (
              <label htmlFor="org-logo-upload-inline" className="cursor-pointer flex items-center gap-3 rounded-lg border-2 border-dashed border-[hsl(var(--dd-rule))] hover:border-[hsl(var(--dd-accent))] hover:bg-[hsl(var(--dd-accent-light))]/30 px-3 py-2.5 transition-colors max-w-xs">
                <div className="h-10 w-10 rounded-lg bg-[hsl(var(--dd-fog))] flex items-center justify-center shrink-0">
                  <Upload className="h-4 w-4 text-muted-foreground" />
                </div>
                <div>
                  <div className="text-sm font-medium text-foreground">{uploading ? 'Uploading…' : 'Upload logo'}</div>
                  <div className="text-[11px] text-muted-foreground font-normal">PNG, JPG, SVG · max 2MB</div>
                </div>
                <input id="org-logo-upload-inline" type="file" accept="image/*" className="hidden" onChange={handleLogoUpload} />
              </label>
            ) : (
              <div className="text-sm text-muted-foreground font-normal">No logo uploaded</div>
            )}
          </Field>
        </div>
      </div>
