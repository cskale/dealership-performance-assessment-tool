...
| 5 | Wire i18n into sidebar navigation | AppSidebar now uses `t()` for all nav labels + section headers. Added 12 new `nav.*` translation keys across all 5 languages (EN, DE, FR, ES, IT). Language change from Account → Preferences now visibly updates sidebar. | pending |
