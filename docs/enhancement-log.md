...
| 6 | Fix i18n dual-system desync | `LanguageContext.setLanguage` now syncs `react-i18next` via `i18n.changeLanguage()` and both localStorage keys (`app_language` + `language`). Profile load also syncs. `i18n.ts` init reads `app_language` first. `changeLanguage` guarded with `isInitialized` check to prevent crash on early mount. | `8789c2c`, `af956e7` |
